...
bool ReadDegrees(const std::string &basename)
{
    ...
    res += fread(degrees.data(), sizeof(size_t), vertex_count, file_in);
...
void WriteDegrees(const std::string &basename)
{
    ...
    size_t res = 0;
    res += fwrite(degrees.data(), sizeof(size_t), vertex_count, file_out);
...
void ViewPacking(Graph *graph, int hop, const std::string &graph_filename, const std::string &partition_filename, int threads_num, size_t max_view_bin_size, int pool_size, int opt, int dump)
{
    ...
    if (suffix.find('x') == std::string::npos)
    {
        ...
        if (opt)
        {
            ...
            {
                std::sort(vertex_chunks[i].begin(), vertex_chunks[i].end(), [](const auto &a, const auto &b)
    ...
    else
    {
        ...
        for (int i = 0; i < first_layer; i++)
            for (int j = 0; j < second_layer; j++)
            {
                if (opt)
                {
                    std::sort(partition[i][j].begin(), partition[i][j].end(), [](const auto &a, const auto &b)
                              { return degrees[a] > degrees[b]; });
                }
    ...
    std::sort(view_bins_id_size.begin(), view_bins_id_size.end(), [](const auto &a, const auto &b)
    ...
    if (dump)
    {
        // write view bin stats to file
        {
            ...
            std::cout << "vertex_sum=" << vertex_sum << ", vertex_count=" << vertex_count << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    Graph *graph = new Graph(graph_filename);
