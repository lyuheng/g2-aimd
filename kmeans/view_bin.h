#ifndef KMEANS_VIEW_BIN_H
#define KMEANS_VIEW_BIN_H
...
public:
    ...
    size_t GetKHopViewSize(uintV v, uintE *row_ptrs, uintV *cols, int hop, std::vector<bool> &added, 
                            std::vector<bool> &visited, std::vector<uintV> &added_vertices, 
                            std::vector<uintV> &visited_vertices)
    {
        size_t size = 0;
        if (added[v] == false)
