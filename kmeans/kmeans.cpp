...
public:
    ...
    KMeans &Initialize(int threads_num = 1)
    {
        ...
        size_t F = graph->GetVertexCount();
        size_t V = view.GetVertexCount();
        centroids.resize(K);
    ...
    int Run2x(uintV *view_vids, size_t nvids, int cur_level, int nlevel, int max_iter, int threads_num = 1)
    {
        ...
        int c0 = bool(min_idx & 2); // (00 01 10 11)& 10 = (00 00 10 10)
        int c1 = bool(min_idx & 1); // (00 01 10 11)& 01 = (00 01 00 01)
        this->centroids.push_back(std::move(centroids0[1 - c0])); // 00000 11111 00000 11111 // 1111 0000 0000 1111
        if (c0 != 1)
        {
        ...
        for (int level = 0, nKs = Ks.size(); level < nKs; ++level)
        {
            ...
            for (int c = 0, n = partitioned_vertices.size(); c < n; c++)
            {
                ...
                if (reorder_centroids)
                {
                    ...
                    std::sort(cids_new2old.begin(), cids_new2old.end(), [&](auto a, auto b)
                              { return num_non_zero_entries[a] > num_non_zero_entries[b]; });
...
int main(int argc, char **argv)
{
    ...
    int reorder_centroids = cmd.GetOptionIntValue("-rc", 1);
