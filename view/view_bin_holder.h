#ifndef VIEW_VIEW_BIN_HOLDER_H
#define VIEW_VIEW_BIN_HOLDER_H
...
public:
    ...
    ViewBinHolder(size_t vc, size_t ms) : vertex_count_(vc), max_view_bin_size_(ms)
    {
        ...
        // FIXME: TODO: figure out why this error is happening:
        // If use the cudaMallocHost, in Transfer(.) this address is looking like being deleted, causing segmentation fault...
        ...
        // This works fine, but may not the best way to do, we always want to pin the memory page and use DMA
        // row_ptrs_ = (uintE *) malloc((vertex_count_ + 1) * sizeof(uintE));
        // cols_ = (uintV *) malloc(max_view_bin_size_);
    }
