#ifndef VIEW_VIEW_BIN_MANAGER_H
#define VIEW_VIEW_BIN_MANAGER_H
...
public:
    ...
    void LoadViewBinPartition(const std::string &filename)
    {
        ...
        // {
        // //     // if (i < 5)
        // //     //     view_bin_partition_map_[i] = 0;
        // //     // else if (i < 10)
        // //     //     view_bin_partition_map_[i] = 1;
        // //     // else if (i < 15)
        // //     //     view_bin_partition_map_[i] = 2;
        // //     // else if (i < 20)
        // //     //     view_bin_partition_map_[i] = 3;
        // //     // else 
        // //     //     view_bin_partition_map_[i] = 4;
        ...
        for (ui i = 0; i <view_bin_num_; i++)
            std::cout << "Source " << i << ": " << partitoned_sources[i].size() << std::endl;
    ...
    void Split(int device_num, int times)
    {
        ...
        while (split_num--)
        {
            ...
            std::sort(sources_factors.begin(), sources_factors.end(), [](const auto &a, const auto &b)
        ...
        std::sort(view_bin_pool_.begin(), view_bin_pool_.end(), [](const auto &a, const auto &b)
                  { return a->GetComputationFactors() > b->GetComputationFactors(); });
        for (int i = 0; i < view_bin_num_; i++)
            std::cout << view_bin_pool_[i]->GetId() << " " << view_bin_pool_[i]->GetComputationFactors() << std::endl;
    }
